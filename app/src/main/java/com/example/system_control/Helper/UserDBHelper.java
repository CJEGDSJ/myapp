package com.example.system_control.Helper;
...
public class UserDBHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (newVersion > 1) {
            ...
            String alter_sql = "ALTER TABLE " + TABLE_NAME + " ADD COLUMN " + "uid VARCHAR;";
    ...
    public long insert(List<UserInfo> infoList) {
        ...
        /*    if (info.name != null && info.name.length() > 0) {
            ...
            if (info.uid != null && info.uid.length() > 0) {
                String condition = String.format("uid='%s'", info.uid);
                tempList = query(condition);
                if (tempList.size() > 0) {
                    update(info, condition);
                    result = tempList.get(0).rowid;
    ...
    public List<UserInfo> query(String condition) {
        String sql = String.format("select rowid,_id," +
                "update_time,uid,password " +
        ...
        while (cursor.moveToNext()) {
            ...
            info.update_time = cursor.getString(2); // 取出字符串
            info.uid = cursor.getString(3); // 取出字符串
    ...
    // 根据手机号码查询指定记录
    public UserInfo queryByPhone(String uid) {
        UserInfo info = null;
        List<UserInfo> infoList = query(String.format("uid='%s'", uid));
